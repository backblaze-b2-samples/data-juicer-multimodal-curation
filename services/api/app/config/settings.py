...
class Settings(BaseSettings):
    # B2 region is the single source of truth for the endpoint - the S3
    # endpoint is derived from it (see `b2_endpoint` below), never hardcoded.
    # Empty by default so no region literal ships in source; startup validation
    # (see main.py) fails fast with an actionable message when it is unset.
    b2_region: str = ""
    ...
    # (the IPv4 loopback Playwright and many browsers use instead of the
    # `localhost` hostname). Production deploys should override with the
    ...
    def b2_endpoint(self) -> str:
        """The S3-compatible endpoint, derived from `b2_region`.

        Region is the single source of truth (B2 standard: no hardcoded region
        literals in source). An empty region yields an obviously-invalid host
        that startup validation rejects before any B2 call is attempted.
        """
